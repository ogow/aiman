...
- `aiman profile create <name> --scope project|user --provider codex|gemini --mode safe|yolo --model <id> --description <text> [--instructions <text>] [--force] [--json]`
- `aiman profile migrate [--json]`
...
The human TTY surfaces are now Ink-based. `aiman` with no arguments opens the default app, and `aiman sesh top` opens the interactive run dashboard. Both require a real TTY, stay small-terminal-first, and reuse the same shared `src/ui/` layer.

For `aiman profile create`, `--scope`, `--provider`, `--mode`, `--model`, and `--description` are required. Instructions can come from `--instructions` or from stdin, which keeps multiline authoring scriptable and avoids hidden interactive prompts that could block parent agents.
...
Profile frontmatter must declare `mode: safe | yolo`. `aiman run` uses that declared mode as the profile's execution mode, and built-in or migrated records may still expose `permissions` as a compatibility alias when older data is being read.

Profile frontmatter can also declare an optional YAML `skills:` list. `aiman profile create` does not scaffold or edit that field yet; add it manually when a profile expects provider-native skills. `aiman run` records those declared names in the launch snapshot for debugging, but leaves actual skill discovery and use to the downstream provider CLI.

Profile frontmatter can also declare an optional YAML `contextFiles:` list of repo-relative file paths. `aiman run` validates those paths, fails fast for missing or unsafe entries, and appends only those file contents as an explicit `Project Context` section in the rendered prompt. This is the supported opt-in path for repo-specific guidance; `aiman` does not automatically inherit the repo `AGENTS.md`. For stable neutral repo context, prefer a small baseline file such as `docs/agent-baseline.md` over putting task-steering behavior in shared router files.

Profile frontmatter can also declare an optional YAML `requiredMcps:` list. `aiman run` checks those MCP names through the selected provider CLI before launch and fails fast when a required MCP is missing, disabled, or reported disconnected.

`--reasoning-effort` is a provider-specific option:

- Codex-backed profiles map it to Codex CLI config as `model_reasoning_effort`.
- On Windows, Codex-backed runs also add `allow_login_shell=false` and `shell_environment_policy.experimental_use_profile=false` so non-interactive runs do not inherit user PowerShell profile behavior.
- Codex-backed runs also add `project_doc_max_bytes=0`, `project_doc_fallback_filenames=[]`, `developer_instructions=""`, `instructions=""`, and `agents={}` so repo `AGENTS.md`, project Codex prompt-shaping instructions, and repo-defined Codex agent roles do not leak into authored `aiman` profiles, while project `.codex` MCP definitions still load.
- Gemini-backed runs also add a child-local settings overlay, passed only to the spawned Gemini process through `GEMINI_CLI_SYSTEM_SETTINGS_PATH`, that forces Gemini `context.fileName` to an impossible filename. That keeps project context files such as `AGENTS.md` or `GEMINI.md` from leaking into authored `aiman` profiles while project `.gemini/settings.json` MCP definitions still load.
- Gemini-backed profiles do not support it and will fail validation at run time.
...
- `aiman` with no args is the default Ink app for humans working in a real TTY.
...
- `aiman run stop <id>` is the quick operator path for stopping one active run by run id without opening the TTY dashboard, including Windows runs launched through npm-style `.cmd` wrappers.
...
- `aiman sesh top` is the interactive Ink session dashboard, defaults to `--filter active`, supports `--filter historic` and `--filter all`, and requires a real TTY. It opens in a lighter list-first view, uses Enter to drill into one run, lets operators stop the selected active run with `s`, and gives the detail view its own scroll controls (`j/k`, arrows, Page Up/Page Down, Home/End, or Space) so long final answers and logs can be read without changing the selected run.
...
`run.md` stores structured execution fields such as `runId`, `status`, `profile`, `profileScope`, `profilePath`, `provider`, `launchMode`, `model`, optional `reasoningEffort`, `mode`, `projectRoot`, timestamps, exit state, and optional `usage`, plus any authored frontmatter like `kind`, `summary`, `artifacts`, or task-specific metadata.
...
When a profile declares skills or `contextFiles`, the same launch snapshot records the declared skill names and the validated context-file paths that were attached at launch time.
...
`npm run test:provider-contract` is the live provider smoke-test suite. It uses the real Codex and Gemini CLIs, skips explicitly when a CLI or auth is unavailable, and checks that ambient repo instruction files stay out while declared `contextFiles` still appear.
