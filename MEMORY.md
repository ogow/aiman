...
- `aiman` does not own a skill catalog, skill CLI surface, or prompt-time skill injection; providers discover skills natively from the repo when they need them.
...
- Authored agents must declare `provider`, `model`, `mode`, `description`, `reasoningEffort`, and a Markdown body containing `{{task}}`.
