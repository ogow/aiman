...
- `aiman` with no arguments opens the interactive app and requires a real TTY.
...
- `aiman profile migrate` is the forward path from legacy `.aiman/agents/*.md` files into `.aiman/profiles/*.md`.
...
- Authored profiles must declare `provider`, `model`, `mode`, `description`, and a Markdown body containing `{{task}}`.
...
- Run/session state is now global: `aiman` stores run directories under `~/.aiman/runs/`, indexes them in `~/.aiman/aiman.db`, and records each run's `projectRoot` so `sesh list`, `show`, `logs`, `inspect`, and `top` work from any working directory without scanning per-project `.aiman/runs/`.
...
- `npm run test:provider-contract` is the live provider smoke-test suite; it uses the real Codex and Gemini CLIs to verify that ambient repo instruction files stay out while explicit `contextFiles` still appear.
...
- Active runs can be stopped through `aiman run stop <run-id>` or the default interactive app; both write a persisted `.stop-requested` marker that the supervising worker polls so stop behavior works cross-platform, including PowerShell/Windows and `.cmd`-wrapped provider process trees.
- Human TTY surfaces now use a shared Ink + `@inkjs/ui` layer under `src/ui/`, while `aiman` with no args and `aiman sesh top` both remain real-TTY-only and small-terminal-first.
- The default interactive app keeps view-based navigation for home, task, run, answer, logs, prompt, details, history, and skills instead of fixed split panes.
- The default interactive app now uses mnemonic letter navigation (`g/a/t/r/i/o/l/p` as applicable), a branded ASCII home screen, and compact shared header chrome instead of numeric view shortcuts and a metrics-heavy landing page.
