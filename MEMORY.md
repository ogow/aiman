...
- `examples/plan-harness.ts` is the minimal plan-execution harness: it runs human-written JSON plans through a builder agent, deterministic check commands, one check-failure retry by default, conditional task review, final integration review, and a persisted `.aiman/harness-runs/` report.
