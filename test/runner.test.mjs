...
test("RunManager executes a visible project agent and records logs", async () => {
  ...
  assert.ok(stdoutEvents.some((event) => event.payload.text.includes("test-model")));
  assert.ok(stdoutEvents.some((event) => event.payload.text.includes("Hello from the test run.")));
});
...
test("RunManager marks long runs as failed when they exceed timeout", async () => {
  ...
  const runManager = new RunManager({ rootDir: workspaceDir, agentRegistry, runStore, killGraceMs: 50 });
  const run = await runManager.spawnRun({
