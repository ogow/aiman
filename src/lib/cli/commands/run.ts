    ...
    (yargs: Argv) =>
      yargs
        .command(
          ...
          (child: Argv) =>
            child
              ...
              .option("write-scope", {
                type: "string",
                array: true,
                describe: "Repeatable write scope path or comma-separated list."
              })
              .option("timeout-ms", {
                type: "string",
          ...
          async (
            ...
            const result = await getApp(context).actions.spawnRun({
              ...
              timeoutMs:
                parsePositiveInteger(argv.timeoutMs, "--timeout-ms") ?? null,
        ...
        .command(
          ...
          async (
            argv: ArgumentsCamelCase<{ runId: string; timeoutMs?: string }>
          ) => {
            const result = await getApp(context).actions.waitForRun({
              runId: argv.runId,
              timeoutMs:
                parsePositiveInteger(argv.timeoutMs, "--timeout-ms") ?? 30000
            });
