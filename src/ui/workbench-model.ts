...
export const workspaceTabOptions = [
   ...
   {
      description: "Choose a profile and start a task.",
      name: "Tasks",
      value: "tasks"
   },
   {
...
function getDuration(run: RunInspection): string {
...
export function buildRunOptions(runs: RunInspection[]): Array<{
   description: string;
   name: string;
   value: string;
}> {
...
export function buildProfileSummary(input: {
   ...
   if (input.profile === undefined) {
      return "No profile available. Create one with `aiman profile create`.";
   }

   const profile = input.profile;
   const mode = profile.mode ?? "safe";
   const header = renderLabelValueBlock([
      ...
      {
         label: "Rights",
         value: formatRunRights(profile.provider, mode)
      },
      { label: "Project", value: input.projectTitle },
      {
         ...
         value:
            input.projectContext !== undefined
               ...
               : "No AGENTS runtime context attached"
      },
      {
         label: "Skills",
         value:
            Array.isArray(profile.skills) && profile.skills.length > 0
               ? profile.skills.join(", ")
               : "No profile-local skills declared"
      }
   ]);
...
export function buildRunSummary(run: RunInspection | undefined): string {
   ...
   const summary = renderLabelValueBlock([
      ...
      { label: "Profile", value: getRunShortLabel(run) },
