...
import { followRunOutput, type RunOutputStream } from "../lib/run-output.js";
...
export function builder(yargs: Argv): Argv {
   return yargs
      ...
      .example("$0 runs logs run-id", "Read the recent output for a run")
      .example(
         "$0 runs logs run-id -f",
...
export async function handler(
   ...
   const content = await (
      await createAiman()
   ).runs.readOutput(args.runId, stream, tail);
