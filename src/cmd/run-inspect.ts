...
export function builder(yargs: Argv): Argv {
   return yargs
      ...
      .example(
         "$0 runs inspect reviewer-1234abcd",
         "Show detailed parsed run information"
      )
      .example(
         "$0 runs inspect reviewer-1234abcd --stream prompt",
...
export async function handler(
   ...
   if (args.stream) {
      const content = await (
         await createAiman()
      ).runs.inspectFile(args.runId, args.stream);
   ...
   const run = await (await createAiman()).runs.get(args.runId);
