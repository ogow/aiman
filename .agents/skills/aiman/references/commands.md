...
- Stop one active run: `aiman agent stop <runId>`
...
- This is the non-TTY stop surface for wrappers, agents, and automation.
- It stops one active run, not all runs for an agent name.
- On Windows, this also handles runs launched through `.cmd` and `.bat` provider wrappers.
...
- For git URLs, `aiman` clones the repo's default branch.
...
- this reads from the global `~/.aiman` run store instead of a project-local runs directory
...
- session lookups resolve runs from the global `~/.aiman` store, so they still work outside the original project cwd
